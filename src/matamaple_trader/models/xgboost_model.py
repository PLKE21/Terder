...
class XGBoostModel:
    ...
    def predict_output(self, X) -> ModelOutput:
        if self.contract.target_type == TaskType.REGRESSION:
            ...
            return ModelOutput(raw,None,None,"xgboost",self.contract.label_version)

        raw=np.asarray(self.estimator.predict(X,output_margin=True))
        ...
        return ModelOutput(raw,proba,classes,"xgboost",self.contract.label_version)
